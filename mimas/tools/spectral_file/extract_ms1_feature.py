...
def process_mzml_file(file_input, ifSciex = True, ifdebug = False):
    ...
    if len(ms_file["ms_list_ms2"]) == 0:
        print("there is no ms2 spectra in such file: "+file_input)
        # open(path_output/"no_ms2", "wt").close()
        return 0

    # Process features
    # print("i am finding features")
    features_all, features_mapped = find_features(ms_file)
    # print('i am done finding features')
    if ifdebug == True:
        print('im in debug mode')
        return(features_mapped)
    features_mapped.dropna(subset=["feature_idx"], inplace=True)
    # features_mapped=features_mapped[features_mapped['charge_offset']==0]
    features_mapped = features_mapped[abs(features_mapped["mz_offset"]) <= 0.1]#can be changed
    ...
    # print('i am extracting ms2 spectra')
    ms2_selected= extract_ms2_spectra(ms_file, features_mapped)
    ms2_selected.drop_duplicates(subset=['scan_number'], inplace = True)
    ms2_selected
...
def extract_ms2_spectra(ms_file, features_mapped: pd.DataFrame):
    ...
    for i, row in features_mapped_selected.iterrows():
        intensity_precursor_ion = _extract_ms1_intensity(
            # ms_file, int(row["ms1_idx"]), row["mz_matched"] -0.01, row["mz"] +0.01
            ms_file, int(row["ms1_idx"]), row["mz"] *(1-ms1_ppm/1E6), row["mz"] *(1+ms1_ppm/1E6)
            )
        ...
        if intensity_select_window == 0 or np.isnan(intensity_select_window) or np.isnan(intensity_precursor_ion):
            ...
            features_mapped_selected.loc[i, "precursor_intensity"] = -1#i added this
        else:
    ...
    selected_ms2 = {
        ...
        'mz_matched':features_mapped_selected['mz_matched'],
        "query_idx":features_mapped_selected['query_idx'],
        "rt_start":features_mapped_selected['rt_start'],
        "rt_apex":features_mapped_selected['rt_apex'],
        "rt_end":features_mapped_selected['rt_end'],
        "int_sum":features_mapped_selected['int_sum'],
        "int_apex":features_mapped_selected['int_apex'],
...
def find_features(query_data):
    ...
    print(f'Repeating hill extraction with centroid_tol {score_median+score_std*3:.2f}')

    hill_ptrs, hill_data, path_node_cnt, score_median, score_std = feature_finding.extract_hills(
        query_data, max_gap, score_median + score_std * 3)

    print(f'Number of hills {len(hill_ptrs):,}, len = {np.mean(path_node_cnt):.2f}')

    hill_ptrs, hill_data = feature_finding.remove_duplicate_hills(hill_ptrs, hill_data, path_node_cnt)
    print(f'After duplicate removal of hills {len(hill_ptrs):,}')
    # return(hill_ptrs, hill_data)
    ...
    print(f'After split hill_ptrs {len(hill_ptrs):,}')
    ...
    print(f'After filter hill_ptrs {len(hill_ptrs):,}')
    ...
    print('Extracted {:,} isotope patterns.'.format(len(isotope_charges)))
